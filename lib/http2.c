...
static void
lws_h2_state_transition(struct lws *wsi, enum lws_h2_states s)
{
...
lws_wsi_server_new(struct lws_vhost *vh, struct lws *parent_wsi,
			    unsigned int sid)
{
	struct lws *wsi = lws_create_new_server_wsi(vh);
...
lws_h2_rst_stream(struct lws *wsi, uint32_t err, const char *reason)
{
	...
	h2n->type = LWS_H2_FRAME_TYPE_COUNT; /* ie, IGNORE */
	lws_h2_state_transition(wsi, LWS_H2_STATE_CLOSED);

	return 0;
}

int
lws_h2_settings_payload(struct lws *wsi, struct http2_settings *settings,
			unsigned char *buf, int len)
{
	...
	while (len >= LWS_H2_SETTINGS_LEN) {
		...
		case H2SET_INITIAL_WINDOW_SIZE:
			...
			lws_start_foreach_ll(struct lws *, w, nwsi->u.h2.child_list) {
				lwsl_info("%s: adjusting child tc cr %d +%d -> %d",
					    __func__,
		       ...
		       unsigned int sid, unsigned int len, unsigned char *buf)
{
	...
	*p++ = sid;
	...
	lwsl_info("%s: %p (eff %p). typ %d, fl 0x%x, sid=%d, len=%d, txcr=%d, nwsi->txcr=%d\n",
		  __func__, wsi, nwsi, type, flags, sid, len, wsi->u.h2.tx_cr,
		  nwsi->u.h2.tx_cr);
...
lws_h2_parse_frame_header(struct lws *wsi)
{
	...
	case LWS_H2_FRAME_TYPE_DATA:
		...
		lwsl_info("Frame header LWS_H2_FRAME_TYPE_DATA\n");
	...
	case LWS_H2_FRAME_TYPE_RST_STREAM:
		...
		lws_h2_state_transition(h2n->swsi, LWS_H2_STATE_CLOSED);
		break;
	...
	case LWS_H2_FRAME_TYPE_HEADERS:
		lwsl_info("LWS_H2_FRAME_TYPE_HEADERS: frame header: sid = %d\n",
			  h2n->sid);
		if (!h2n->sid) {
...
update_end_headers:
		...
		h2n->swsi->u.h2.END_HEADERS = !!(h2n->flags & LWS_H2_FLAG_END_HEADERS);
		if (h2n->swsi->u.h2.END_HEADERS)
			h2n->cont_exp = 0;
		lwsl_debug("END_HEADERS %d\n", h2n->swsi->u.h2.END_HEADERS);

		if (h2n->type != LWS_H2_FRAME_TYPE_CONTINUATION) {
...
lws_h2_parse_end_of_frame(struct lws *wsi)
{
	...
	case LWS_H2_FRAME_TYPE_HEADERS:
		...
		case LWS_H2_STATE_IDLE:
			lws_h2_state_transition(h2n->swsi, LWS_H2_STATE_OPEN);
			break;
		case LWS_H2_STATE_RESERVED_REMOTE:
			lws_h2_state_transition(h2n->swsi,
		...
		if (lws_hdr_extant(h2n->swsi, WSI_TOKEN_HTTP_CONTENT_LENGTH))
			h2n->swsi->u.h2.incoming_content_length = atoll(
		...
		if (h2n->swsi->u.h2.h2_state ==
				LWS_H2_STATE_HALF_CLOSED_REMOTE ||
		    h2n->swsi->u.h2.h2_state == LWS_H2_STATE_CLOSED) {
			lws_h2_goaway(wsi, H2_ERR_STREAM_CLOSED,
				"Banning service on LWS_H2S_HALF_CLOSED_REMOTE");
			break;
		}
		...
		if (n && lws_http_transaction_completed(h2n->swsi))
			return 1;
		break;

	case LWS_H2_FRAME_TYPE_DATA:
		h2n->swsi->u.h2.incoming_content += h2n->length;
		if (h2n->swsi->u.h2.incoming_content_length &&
		    h2n->swsi->u.h2.incoming_content_length !=
			h2n->swsi->u.h2.incoming_content &&
		    (wsi->u.h2.END_STREAM ||
		     h2n->swsi->u.h2.incoming_content >
			h2n->swsi->u.h2.incoming_content_length)) {
			lws_h2_rst_stream(h2n->swsi, H2_ERR_PROTOCOL_ERROR,
					  "Rx content length mismatch");
			break;
		}
		if (wsi->u.h2.END_STREAM &&
	...
	case LWSS_HTTP2_ESTABLISHED:
		...
		case LWS_H2_FRAME_TYPE_SETTINGS:
			...
			if (n != LWS_H2_SETTINGS_LEN - 1)
				break;
			lws_h2_settings_payload(wsi, &h2n->set,
						h2n->one_setting,
						LWS_H2_SETTINGS_LEN);
			break;
		...
		case LWS_H2_FRAME_TYPE_GOAWAY:
			...
			default:
				if (h2n->inside - 9 < sizeof(h2n->goaway_str) - 1)
